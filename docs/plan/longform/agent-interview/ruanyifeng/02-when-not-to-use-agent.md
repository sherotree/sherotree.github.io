...
> 系列：上一篇：[理解 Agent 边界](./01-what-is-agent.md) · [地图](./00-series-map.md) · 下一篇：[理解 ReAct 与 Plan-and-Execute](./03-react-vs-plan-execute.md)

学会定义 Agent 之后，下一题往往更锋利：什么时候不要用它。

这题考的不是你会不会喊新技术，而是你会不会克制。任务路径能写清、失败代价高、或必须可复现时，把下一步决策权交给模型，常常得不偿失。

本文整理三个我会明确否决的场景、一套上线前检查表，以及面试里常见追问。
...
不难看出，否决的核心不是「模型够不够聪明」，而是「控制权该不该交给运行时的模型探索」。
...
每一步输入输出清晰，异常码也清晰。用 Agent 让模型自己「探索」流程，只会增加随机性与费用。
...
「客服里 80% 标准问答 + 20% 开放问题」：标准路径走 Workflow / 检索问答；开放路径再进 Agent，并加步数与权限顶。
...
可以，但要标明是探索原型；上线门禁单独过否决清单，不把 Demo 架构直接当生产。

**Q2：开源 Coding Agent 为什么到处都是 Agent？**  
写代码任务路径高度开放，环境反馈密（编译 / 测试），失败常可回滚（git）。符合 Agent 优势区；不等于所有业务都能照搬。

**Q3：否决后怎么说服鼓吹 Agent 的同事？**  
对齐失败成本与指标（差错率、审计、成本），给替代方案与试点边界，而不是道德批判。

**Q4：工具很多但权限都很大，怎么办？**  
先砍权限与 HITL，再谈 Agent；否则是否决「裸 Agent」，不是否决自动化。
...
下一篇讲两种常见循环策略：ReAct 和 Plan-and-Execute，怎么选。
